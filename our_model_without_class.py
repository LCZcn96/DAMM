...
from utils import set_seed, PCGrad, ImprovedDeepSurvLoss, EarlyStopping
...
def detach_and_grad(inputs) -> Tuple[torch.Tensor, ...]:
...
def train_and_evaluate(data_dict, config):
    ...
    for fold, (train_indices,
        ...
        with torch.no_grad():
            for task_inputs1, task_inputs2, task_inputs3, task_inputs4, label in val_loader:
                ...
                all_labels.append([label[-3], label[-2]])
        ...
        with open(f"./without_class/metrics_fold_{fold}.json",
                  'w',
    ...
    with open("./without_class/avg_metrics.json", 'w',
              encoding='utf-8') as json_file:
        json.dump(avg_metrics, json_file, ensure_ascii=False, indent=4)

    with open("./without_class/all_fold_metrics.json", 'w',
              encoding='utf-8') as json_file:
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'Dropout': 0,
        'task_feat_nums': [3, 3],
        'criterion_list': [ImprovedDeepSurvLoss(),
                           nn.L1Loss()],
        'lr': 1e-4,
        'n_splits': 5,
        'batch_size': 1024,
