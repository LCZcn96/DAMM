...
def load_data():
    ...
    rnaseq_feature_name, methy_feature_name, mirna_feature_name, scnv_feature_name, merged_data = preprocess_data(
        rnaseq_data, methy_data, mirna_data, scnv_data, clinical_data)
...
class CancerOmicsDataset(Dataset):
    ...
    def __init__(self, rnaseq_data, methy_data, mirna_data, scnv_data,
                 *labels):
    ...
    def __getitem__(self, idx):
        return self.rnaseq_data[idx], self.methy_data[idx], self.mirna_data[
            idx], self.scnv_data[idx], [label[idx] for label in self.labels]
...
def get_cancer_datasets():
    ...
    survival_labels = cancer_data_dict["merged_data"][[
    ...
    rnaseq_data = cancer_data_dict["merged_data"][
        cancer_data_dict["rnaseq_feature_name"]]
    methy_data = cancer_data_dict["merged_data"][
        cancer_data_dict["methy_feature_name"]]
    mirna_data = cancer_data_dict["merged_data"][
        cancer_data_dict["mirna_feature_name"]]
    scnv_data = cancer_data_dict["merged_data"][
        cancer_data_dict["scnv_feature_name"]]
    ...
    dataset = CancerOmicsDataset(rnaseq_data, methy_data, mirna_data,
                                 scnv_data, domain_labels, cancer_type,
                                 survival_labels, age_labels, stage_labels)
